...
from LuckyXMusic.utils.decorators.admins import ActualAdminCB
from LuckyXMusic.utils.decorators.language import language, languageCB
from LuckyXMusic.utils.inline.settings import (
    auth_users_markup,
    playmode_users_markup,
    setting_markup,
    vote_mode_markup,
)
from LuckyXMusic.utils.inline.start import private_panel
...
async def settings_mar(client, message: Message, _):
    ...
    await message.reply_text(
        _["setting_1"].format(Lucky.mention, message.chat.id, message.chat.title),
        reply_markup=InlineKeyboardMarkup(buttons),
    )


@Lucky.on_callback_query(filters.regex("settings_helper") & ~BANNED_USERS)
@languageCB
...
@Lucky.on_callback_query(filters.regex("settingsback_helper") & ~BANNED_USERS)
@languageCB
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        await Lucky.resolve_peer(OWNER_ID)
        OWNER = OWNER_ID
        buttons = private_panel(_)
        return await CallbackQuery.edit_message_text(
            _["start_2"].format(CallbackQuery.from_user.mention, Lucky.mention),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
        buttons = setting_markup(_)
...
@Lucky.on_message(filters.command(["repo", "source"]) & filters.private)
async def send_repo(_, message: Message):
    await message.reply_video(
        ...
        caption=(
            ...
            "🧡 ᴄʀᴇᴅɪᴛꜱ : <a href='https://t.me/LuckyXUpdate'>@LuckyXUpdate</a>"
        ),
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("🎵 Music + Management Bot Repo", url="https://github.com/The-LuckyX/LuckyMusic")],
                [InlineKeyboardButton("🎧 Only Music Bot Repo", url="https://github.com/The-LuckyX/LuckyXMusic")]
            ]
        ),
...
@Lucky.on_callback_query(filters.regex("FERRARIUDTI") & ~BANNED_USERS)
@ActualAdminCB
...
@Lucky.on_callback_query(filters.regex(pattern=r"^(AUTH|AUTHLIST)$") & ~BANNED_USERS)
@ActualAdminCB
    ...
    if command == "AUTHLIST":
        ...
        if not _authusers:
            ...
            for note in _authusers:
                ...
                try:
                    user = await Lucky.get_users(user_id)
...
@Lucky.on_callback_query(filters.regex("VOMODECHANGE") & ~BANNED_USERS)
@ActualAdminCB
