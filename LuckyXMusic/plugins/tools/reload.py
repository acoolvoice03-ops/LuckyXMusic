...
from LuckyXMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from LuckyXMusic.utils.decorators import ActualAdminCB, AdminActual, language
from LuckyXMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def reload_admin_cache(client, message: Message, _):
    try:
        ...
        async for user in Lucky.get_chat_members(
...
@Lucky.on_message(filters.command(["reboot"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    mystic = await message.reply_text(_["reload_4"].format(Lucky.mention))
    ...
    if chat_id:
        try:
            got = await Lucky.get_chat(chat_id)
        except:
            pass
        ...
        try:
            ...
            pass
    return await mystic.edit_text(_["reload_5"].format(Lucky.mention))


@Lucky.on_callback_query(filters.regex("close") & ~BANNED_USERS)
...
@Lucky.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS)
@ActualAdminCB
