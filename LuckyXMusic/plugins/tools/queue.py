...
from LuckyXMusic.utils import LuckyBin, get_channeplayCB, seconds_to_min
from LuckyXMusic.utils.database import get_cmode, is_active_chat, is_music_playing
from LuckyXMusic.utils.decorators.language import language, languageCB
from LuckyXMusic.utils.inline import queue_back_markup, queue_markup
...
async def get_queue(client, message: Message, _):
    ...
    cap = _["queue_8"].format(Lucky.mention, title, typo, user, send)
    upl = (
...
@Lucky.on_callback_query(filters.regex("GetTimer") & ~BANNED_USERS)
...
@Lucky.on_callback_query(filters.regex("GetQueued") & ~BANNED_USERS)
@languageCB
...
@Lucky.on_callback_query(filters.regex("queue_back_timer") & ~BANNED_USERS)
@languageCB
