...
from LuckyXMusic.utils.database import set_cmode
from LuckyXMusic.utils.decorators.admins import AdminActual
from config import BANNED_USERS


@Lucky.on_message(filters.command(["channelplay"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def playmode_(client, message: Message, _):
    ...
    if (str(query)).lower() == "disable":
        ...
        chat = await Lucky.get_chat(message.chat.id)
        if chat.linked_chat:
        ...
        try:
            chat = await Lucky.get_chat(query)
        except:
