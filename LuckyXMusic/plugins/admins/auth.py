...
from LuckyXMusic.utils import extract_user, int_to_alpha
...
from LuckyXMusic.utils.decorators import AdminActual, language
from LuckyXMusic.utils.inline import close_markup
from config import BANNED_USERS, adminlist


@Lucky.on_message(filters.command("auth") & filters.group & ~BANNED_USERS)
@AdminActual
...
@Lucky.on_message(filters.command("unauth") & filters.group & ~BANNED_USERS)
@AdminActual
...
async def authusers(client, message: Message, _):
    ...
    if not _wtf:
        ...
        for umm in _wtf:
            ...
            try:
                user = (await Lucky.get_users(user_id)).first_name
                j += 1
            except:
                continue
